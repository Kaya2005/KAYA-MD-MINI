...
const BATCH_SIZE = 5;  // Stickers envoyés en parallèle
const BATCH_DELAY = 2000; // ms entre chaque batch

export default {
  name: 'tg',
  alias: ['telegram', 'stickertg'],
  description: 'Télécharge un pack de stickers Telegram et les envoie sur WhatsApp',
  category: 'Fun',
    ...
    try {
      ...
      if (!url) {
        return kaya.sendMessage(
          m.chat,
          { text: '⚠️ Fournis l’URL du pack Telegram.\nEx: .tg https://t.me/addstickers/Porcientoreal' },
          { quoted: m }
        );
      }

      // ✅ Regex corrigée pour mobile et PC
      ...
      if (!packData.ok || !packData.result) throw new Error('Pack invalide ou privé');

      const stickers = packData.result.stickers;
      await kaya.sendMessage(
        m.chat,
        { text: `📦 Pack trouvé : ${stickers.length} stickers\n⏳ Téléchargement en cours...` },
        { quoted: m }
      );
      ...
      for (let i = 0; i < stickers.length; i += BATCH_SIZE) {
        ...
        await Promise.all(batch.map(async (sticker, index) => {
          try {
            ...
            console.error(`❌ Erreur sticker ${i + index}:`, err);
          }
        }));
      ...
      await kaya.sendMessage(
        m.chat,
        { text: '❌ Impossible de télécharger le pack. Vérifie l’URL ou la visibilité du pack.' },
        { quoted: m }
      );
    }
  }
